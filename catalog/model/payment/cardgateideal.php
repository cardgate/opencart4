...
class CardGateIdeal extends \Opencart\System\Engine\Model {
    ...
    public function getMethods( $address ) {
        ...
        if ($this->cart->hasSubscription()) {
            $status = false;
        } elseif (!$this->config->get('payment_cardgateideal_geo_zone_id')) {
            $status = true;
        } else {
            $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "zone_to_geo_zone` WHERE `geo_zone_id` = '" . (int)$this->config->get('payment_cardgateideal_geo_zone_id') . "' AND `country_id` = '" . (int)$address['country_id'] . "' AND (`zone_id` = '" . (int)$address['zone_id'] . "' OR `zone_id` = '0')");
            if ($query->num_rows) {
                $status = true;
            } else {
                $status = false;
        ...
        if ($this->config->get('payment_cardgateideal_custom_payment_method_text')){
            $title = trim($this->config->get('payment_cardgateideal_custom_payment_method_text'));
        } else {
        ...
        $method_data = [];
        if ( $status ) {
            $option_data['cardgateideal'] = [
                'code' => 'cardgateideal.cardgateideal',
            ...
            $method_data = [
                ...
                'name'       => $title,
                'option'     => $option_data,
                'sort_order' => $this->config->get('payment_cardgateideal_sort_order')
            ];
        }
