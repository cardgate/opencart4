 ...
 */
class CardGateBanktransfer extends \Opencart\System\Engine\Model {
    ...
    public function getMethods( $address ) {
        ...
        $this->load->language( 'extension/cardgate/payment/cardgatebanktransfer' );
        ...
        if ($this->config->get('payment_cardgatebanktransfer_custom_payment_method_text')){
            $title = trim($this->config->get('payment_cardgatebanktransfer_custom_payment_method_text'));
        } else {
        ...
        $method_data = [];
        if ( $status ) {
            $option_data['cardgatebanktransfer'] = [
                'code' => 'cardgatebanktransfer.cardgatebanktransfer',
