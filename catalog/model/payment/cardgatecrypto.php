 ...
 */
class CardGateCrypto extends \Opencart\System\Engine\Model {

    public function getMethods( $address ) {
        $this->load->language( 'extension/cardgate/payment/cardgatecrypto' );
        ...
        if ($this->config->get('payment_cardgatecrypto_custom_payment_method_text')){
            $title = trim($this->config->get('payment_cardgatecrypto_custom_payment_method_text'));
        } else {
        ...
        $method_data = [];
        if ( $status ) {
            $option_data['cardgatecrypto'] = [
                'code' => 'cardgatecrypto.cardgatecrypto',
