 ...
 */
class CardGateOnlineueberweisen extends \Opencart\System\Engine\Model {
    ...
    public function getMethods( $address ) {
        ...
        $this->load->language( 'extension/cardgate/payment/cardgateonlineueberweisen' );
        ...
        if ($this->config->get('payment_cardgateonlineueberweisen_custom_payment_method_text')){
            $title = trim($this->config->get('payment_cardgateonlineueberweisen_custom_payment_method_text'));
        } else {
        ...
        $method_data = [];
        if ( $status ) {
            $option_data['cardgateonlineueberweisen'] = [
                'code' => 'cardgateonlineueberweisen.cardgateonlineueberweisen',
